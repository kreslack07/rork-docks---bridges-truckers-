...
nonisolated enum CacheService {
    ...
    static func saveHazards(_ hazards: [Hazard]) {
        ...
        UserDefaults.standard.set(data, forKey: hazardsKey)
        UserDefaults.standard.set(Date().timeIntervalSince1970, forKey: hazardsCacheTimestampKey)
    }

    static func loadHazards() -> [Hazard]? {
        guard let data = UserDefaults.standard.data(forKey: hazardsKey),
              let hazards = try? JSONDecoder().decode([Hazard].self, from: data) else { return nil }
        return hazards
    }

    static func hazardsCacheDate() -> Date? {
        let ts = UserDefaults.standard.double(forKey: hazardsCacheTimestampKey)
    ...
    static func saveDocks(_ docks: [Dock]) {
        ...
        UserDefaults.standard.set(data, forKey: docksKey)
        UserDefaults.standard.set(Date().timeIntervalSince1970, forKey: docksCacheTimestampKey)
    }

    static func loadDocks() -> [Dock]? {
        guard let data = UserDefaults.standard.data(forKey: docksKey),
              let docks = try? JSONDecoder().decode([Dock].self, from: data) else { return nil }
        return docks
    }

    static func docksCacheDate() -> Date? {
        let ts = UserDefaults.standard.double(forKey: docksCacheTimestampKey)
        guard ts > 0 else { return nil }
        return Date(timeIntervalSince1970: ts)
    }
